...
class MegaBackend(duplicity.backend.Backend):
    ...
    def download(self, remote_file, local_file):
        ...
        if self._megarc:
            cmd = ['megaget', '--config', self._megarc, '--path', local_file, self._folder + '/' + remote_file]
        else:
            cmd = ['megaget', '-u', self._username, '-p', self._password, '--path', local_file, self._folder + '/' + remote_file]

        try:
            subprocess.call(cmd)
        except Exception as e:
            raise BackendException("megaget failed when downloading file '%s' to '%s'" % (self._folder + '/' + remote_file, local_file))
    ...
    def upload(self, local_file, remote_file):
        ...
        for i in range(5): # give it multiple chances
            if self._megarc:
                cmd = ['megaput', '--config', self._megarc, '--path', self._folder + '/' + remote_file, local_file]
            else:
                cmd = ['megaput', '-u', self._username, '-p', self._password, '--path', self._folder + '/' + remote_file, local_file]

            try:
                if subprocess.call(cmd) == 0:
                    return
                print("megaput failed, retry")
                time.sleep(60)
            except Exception as e:
                raise BackendException("megaput failed when uploading file '%s' to '%s'" % (local_file, self._folder + '/' + remote_file))

        raise BackendException("megaput failed uploading %s" % (local_file))
    ...
    def delete(self, remote_file):
        ...
        try:
            ...
            raise BackendException("megarm failed when deleting file '%s'" % (self._folder + '/' + remote_file,))
